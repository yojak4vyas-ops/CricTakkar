...
- Next task: Wordle data Batch 3 of 5 — Sri Lanka + Pakistan + England (30 players), same verification process. Then Batch 4 (South Africa + Bangladesh + Afghanistan), Batch 5 (Zimbabwe, 10 players). Until all batches are done, only fully-verified countries can be today's mystery player — expands automatically as each batch completes. Also still parked from earlier: Top 5 section (Feature 22), Cricket News Feed (Feature 24), Player Encyclopaedia (Feature 28), Match Day Alert (Feature 26), Push Notifications (Feature 25), Hindi language support (Feature 27), Battle Friends groundwork (Phase 3). If community poems get real usage, revisit moderation — the 3-report auto-hide is a stopgap, not a permanent policy.
