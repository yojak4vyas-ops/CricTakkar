...
- Next task: Wordle data Batch 4 of 5 — South Africa + Bangladesh + Afghanistan (30 players), applying the same dedicated-IPL-search method and sharpened NA standard. Then Batch 5 (Zimbabwe, 10 players) — the final batch, after which all 130 players will be fully verified and the "only fully-verified countries can be today's mystery player" restriction becomes moot. Also still parked from earlier: Top 5 section (Feature 22), Cricket News Feed (Feature 24), Player Encyclopaedia (Feature 28), Match Day Alert (Feature 26), Push Notifications (Feature 25), Hindi language support (Feature 27), Battle Friends groundwork (Phase 3). If community poems get real usage, revisit moderation — the 3-report auto-hide is a stopgap, not a permanent policy.
