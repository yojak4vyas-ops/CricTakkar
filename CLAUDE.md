...
- `bowlingStyle` — added Day 14. Mark "NA" if the player has never bowled in international/first-class cricket (e.g. a specialist keeper-batter). Otherwise use the ESPNcricinfo-style description (e.g. "Right-arm fast", "Left-arm orthodox").
...
- Next task: Wordle data Batch 2 of 5 — Australia + West Indies + New Zealand (30 players), same verification process. Then Batch 3 (Sri Lanka + Pakistan + England), Batch 4 (South Africa + Bangladesh + Afghanistan), Batch 5 (Zimbabwe, 10 players). Until all batches are done, only fully-verified countries can be today's mystery player — expands automatically as each batch completes. Also still parked from earlier: Top 5 section (Feature 22), Cricket News Feed (Feature 24), Player Encyclopaedia (Feature 28), Match Day Alert (Feature 26), Push Notifications (Feature 25), Hindi language support (Feature 27), Battle Friends groundwork (Phase 3). If community poems get real usage, revisit moderation — the 3-report auto-hide is a stopgap, not a permanent policy.
